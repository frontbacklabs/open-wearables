...
Register an application from **Settings → Credentials → SDK Applications** in the developer portal, or via the API:
...
<Tabs>
  ...
  <Tab title="SleepSummary">
    ...
    ```
    `duration_minutes` and `time_in_bed_minutes` exclude naps and use minutes. `sleep_latency_seconds` is the
    sleep-onset latency for the longest main-sleep session and is currently populated only for Oura data.
  </Tab>
