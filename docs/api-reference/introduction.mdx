...
<CardGroup cols={2}>
  ...
  <Card
    title="OpenAPI JSON"
    icon="file-code"
    href="https://github.com/the-momentum/open-wearables/blob/main/docs/openapi.json"
  >
...
| `sort_order` | `asc` | `asc` returns earliest data first, `desc` returns latest first |
...
This reference is generated from [`docs/openapi.json`](https://github.com/the-momentum/open-wearables/blob/main/docs/openapi.json), which is exported from the API code on every change, so it always describes the current `main` branch.

Every Open Wearables instance also serves the spec for the exact version it runs:
- **Swagger UI:** `https://<your-instance>/docs`
- **OpenAPI JSON:** `https://<your-instance>/openapi.json`

<Tip>
  If you self-host an older release, the spec served by your own instance is the accurate reference for it.
</Tip>
