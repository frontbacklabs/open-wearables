...
class TestTimeSeriesServiceGetTimeseries:
    ...
    def test_asc_returns_earliest_first(self, db: Session) -> None:
        ...
        result = self._fetch_page(db, user.id, limit=10)
    ...
    def test_desc_returns_latest_first(self, db: Session) -> None:
        ...
        result = self._fetch_page(db, user.id, sort_order="desc", limit=10)
    ...
    def test_desc_paginates_forward_and_backward(self, db: Session) -> None:
        ...
        page1 = self._fetch_page(db, user.id, sort_order="desc")
        assert [s.timestamp for s in page1.data] == [timestamps[4], timestamps[3]]
        assert page1.pagination.has_more is True
        assert page1.pagination.previous_cursor is None

        page2 = self._fetch_page(db, user.id, cursor=page1.pagination.next_cursor, sort_order="desc")
        ...
        page3 = self._fetch_page(db, user.id, cursor=page2.pagination.next_cursor, sort_order="desc")
        assert [s.timestamp for s in page3.data] == [timestamps[0]]
        assert page3.pagination.has_more is False

        back2 = self._fetch_page(db, user.id, cursor=page3.pagination.previous_cursor, sort_order="desc")
        ...
        back1 = self._fetch_page(db, user.id, cursor=back2.pagination.previous_cursor, sort_order="desc")
    ...
    def test_asc_backward_page_exposes_next_cursor(self, db: Session) -> None:
        ...
        page1 = self._fetch_page(db, user.id, sort_order="asc")
        page2 = self._fetch_page(db, user.id, cursor=page1.pagination.next_cursor, sort_order="asc")

        back1 = self._fetch_page(db, user.id, cursor=page2.pagination.previous_cursor, sort_order="asc")
