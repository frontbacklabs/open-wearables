...
@pytest.fixture
def fast_renewal() -> Generator[None, None, None]:
    with patch.object(sync_coordination, "_renew_interval", return_value=RENEW_INTERVAL):
        yield
...
class TestLeaseTtl:
    def test_pull_scope_uses_the_short_lease(self) -> None:
        ...
        assert sync_coordination._ttl_for("backfill") == sync_coordination._BACKFILL_TTL
        assert sync_coordination._ttl_for("pull") < sync_coordination._ttl_for("backfill")

    def test_renewal_leaves_headroom_for_a_missed_beat(self) -> None:
        assert sync_coordination._renew_interval() * 2 <= settings.linked_sync_pull_lease_seconds

    def test_acquired_pull_lock_carries_the_lease_ttl(self, redis_client: Any) -> None:
        acquired, token, _ = try_become_primary(PROVIDER, PROVIDER_USER_ID, str(uuid4()), scope="pull")
        assert acquired
...
class TestRenewPrimary:
    ...
    def test_renew_fails_once_another_profile_holds_it(self) -> None:
        user_id, token = str(uuid4()), "stale-token"
        try_become_primary(PROVIDER, PROVIDER_USER_ID, str(uuid4()), scope="pull")

        assert renew_primary(PROVIDER, PROVIDER_USER_ID, user_id, token, scope="pull") is False

    def test_renew_retakes_a_lease_that_lapsed_unclaimed(self, redis_client: Any) -> None:
        """Redis unreachable for longer than a lease: the key is gone but nobody took it."""
        user_id, token = str(uuid4()), "still-working"
        key = sync_coordination._primary_key(PROVIDER, PROVIDER_USER_ID, "pull")

        assert renew_primary(PROVIDER, PROVIDER_USER_ID, user_id, token, scope="pull") is True
        assert redis_client.get(key) == f"{user_id}:{token}"


class TestRenewalThread:
    def test_no_thread_without_a_token(self) -> None:
        bind_primary_lease(PROVIDER, PROVIDER_USER_ID, str(uuid4()), "")

        assert sync_coordination._renewer.state is None
    ...
    def test_renews_until_the_lease_is_cleared(self, fast_renewal: None) -> None:
        ...
        with patch.object(sync_coordination, "renew_primary", side_effect=calls.hit(True)):
            bind_primary_lease(PROVIDER, PROVIDER_USER_ID, str(uuid4()), "tok")
            assert calls.reached(2)
            _, thread = sync_coordination._renewer.state
    ...
    def test_stops_renewing_once_another_holder_has_the_lock(self, fast_renewal: None) -> None:
        calls = _Counter()
        with patch.object(sync_coordination, "renew_primary", side_effect=calls.hit(False)):
            bind_primary_lease(PROVIDER, PROVIDER_USER_ID, str(uuid4()), "tok")
            _, thread = sync_coordination._renewer.state
            assert _eventually(lambda: not thread.is_alive())
            assert calls.count == 1  # gave up on the first refusal, no retry storm
    ...
    def test_a_second_bind_replaces_the_first_renewer(self, fast_renewal: None) -> None:
        with patch.object(sync_coordination, "renew_primary", return_value=True):
            bind_primary_lease(PROVIDER, PROVIDER_USER_ID, str(uuid4()), "tok")
            _, first = sync_coordination._renewer.state
            bind_primary_lease(PROVIDER, PROVIDER_USER_ID, str(uuid4()), "tok2")

            assert first.is_alive() is False
            assert sync_coordination._renewer.state[1] is not first

    def test_lease_is_per_thread(self, fast_renewal: None) -> None:
        with patch.object(sync_coordination, "renew_primary", return_value=True):
            bind_primary_lease(PROVIDER, PROVIDER_USER_ID, str(uuid4()), "tok")
            with ThreadPoolExecutor(max_workers=1) as pool:
                # Another task thread sees no lease, and clearing there touches nothing.
                assert pool.submit(lambda: getattr(sync_coordination._renewer, "state", None)).result() is None
                pool.submit(clear_primary_lease).result()
            assert sync_coordination._renewer.state is not None
